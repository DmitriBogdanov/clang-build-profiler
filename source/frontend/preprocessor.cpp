...
void vector_remove_by_predicate(std::vector<T>& vec, Predicate&& predicate) {
    const auto iter = std::remove_if(vec.begin(), vec.end(), predicate);
    vec.erase(iter, vec.end());
}

void prune_uncategorized(std::vector<cbp::tree>& children) {
    const auto predicate = [](const cbp::tree& child) { return child.category == cbp::tree_category::none; };
    vector_remove_by_predicate(children, predicate);
}
...
void prettify_tree(cbp::tree& tree, const cbp::config& config, std::string_view working_directory) {
    ...
    prune_uncategorized(tree.children); // uses categorization for pruning

    // Prune empty targets sometimes generated by CMake
    if (tree.type == cbp::tree_type::targets) {
        const auto predicate = [](const cbp::tree& child) {
            return child.total == cbp::microseconds{} && child.children.empty();
        };
        vector_remove_by_predicate(tree.children, predicate);
    }
...
void cbp::preprocess(cbp::profile& profile, std::string_view working_directory) try {
    ...
    prettify_tree(profile.summary.stages, profile.config, working_directory);
    
    prettify_root(profile.tree, profile.config);
    prettify_root(profile.summary.stages, profile.config);
