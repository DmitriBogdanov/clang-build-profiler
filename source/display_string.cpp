...
private:
    ...
    void serialize_tree(const cbp::tree& tree) {
        ...
        for (const auto& category : this->config.tree.categorize) {
            if (tree.total >= category.duration) {
        ...
        const auto rel_total = cbp::time::to_percentage(tree.total, this->timeframe);
        const auto rel_self  = cbp::time::to_percentage(tree.self, this->timeframe);
        ...
        // Serialize children
        ++this->depth;
        for (const auto& child : tree.children) this->serialize_tree(child);
        --this->depth;
    }
...
public:
    ...
    std::string operator()(const cbp::tree& tree) {
        this->str.clear();
        this->serialize_tree(tree);
        return this->str;
    }
};
...
std::string cbp::display::string::serialize(const cbp::profile& profile, bool colors) try {
    ...
    if (profile.config.tree.enabled) {
        string_serializer serializer{profile.config, colors, profile.tree.total};

        return serializer(profile.tree);
    }

    return {}; // TEMP:

} catch (std::exception& e) {
    // Helps make error messages nicer
    throw cbp::exception{"Could not serialize profile results, error:\n{}", e.what()};
}