...
cbp::merge_summary cbp::create_merge_summary(const cbp::tree& tree) try {
    cbp::tree total_parsing;
    cbp::tree total_instantiation;

    expand_and_merge(total_parsing, total_instantiation, tree);

    return cbp::merge_summary{.parsing = std::move(total_parsing), .instantiation = std::move(total_instantiation)};
